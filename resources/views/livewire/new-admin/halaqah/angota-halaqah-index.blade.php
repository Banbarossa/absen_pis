    ...
    <div  class="mb-6 text-sm font-medium text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700">
        <div>
            <div class="flex items-end justify-between gap-2 mb-4">
                <x-text-input-tailwind wire:model.live='search' placeholder='Type to search....' class="w-full md:inline md:w-1/2 lg:w-1/3"></x-text-input-tailwind>
                <div class="relative flex items-center gap-2" x-data="{dropdown:false}" x-on:click.outside="dropdown = false">
                    <button data-modal-target="extralarge-modal" data-modal-toggle="extralarge-modal" class="block h-auto px-5 py-2.5 text-sm font-medium text-center border border-red-400 rounded-lg md:inline text-red hover:ring-2 hover:ring-red-300 focus:ring-2 focus:outline-none focus:ring-red-300"  type="button">
                        <span>
                            Tambah
                        </span>
                    </button>
                    ...
                    <tbody x-data={open:null}>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <td scope="row" class="items-center table-cell w-3 px-4 py-4 md:hidden">
                                <button x-on:click="open = open === {{ $item->anggota_halaqah_id }} ? null : {{ $item->anggota_halaqah_id }}">
                            ...
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                ...
                                <div class="block md:hidden" x-show="open === {{ $item->anggota_halaqah_id }}" x-on:click.outside="open = null">
                            ...
                            <td class="hidden px-6 py-4 md:table-cell">
                                <button wire:click='hapusAnggotaRombel({{ $item->anggota_halaqah_id }})' wire:key='{{ $item->id }}'>Hapus</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
