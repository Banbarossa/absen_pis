<div class="w-full" x-data="{ popupmengajar: false, imageUrl: '' }">
            ...
            <li class="p-4 py-4 mb-3 border rounded-xl bg-gray-50 {{ $item->kehadiran == 'alpa' ? 'bg-red-100 animate-pulse' :'' }}">
                ...
                <div class="flex items-center justify-start gap-6 ">
                    <div class="text-center">
                        @if ($item->image)
                        <a href="javascript:void(0)" x-on:click="popupmengajar = true; imageUrl = '{{ asset('storage/public/images/' . $item->image) }}'">
                            <img src="{{asset('storage/public/images/'.$item->image)}}" class="w-20 h-20 rounded-xl min-w-20" alt="">
                        </a>
                        @else
                        <a href="javascript:void(0)" x-on:click="popupmengajar = true; imageUrl = '{{ asset('assets/images/avatar.png') }}'">
    ...
    <div x-show="popupmengajar" class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-50 z-50 flex justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-gray-500 bg-opacity-50">
        <div class="relative w-full max-w-lg max-h-full p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" @click.outside="popupmengajar = false">
                <div class="flex items-center justify-between p-4 border-b rounded-t md:p-5 dark:border-gray-600">
                    <h3 class="text-xl font-semibold text-gray-700 dark:text-white">
                        Gambar
                    </h3>
                    <button type="button" x-on:click="popupmengajar = false" class="inline-flex items-center justify-center w-8 h-8 text-sm text-gray-400 bg-transparent rounded-lg hover:bg-gray-200 hover:text-gray-900 ms-auto dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="static-modal">
                ...
                <div class="flex items-center px-4 py-2 border-t border-gray-200 rounded-b md:p-5 dark:border-gray-600">
                    <button
                        x-on:click="popupmengajar = false"
